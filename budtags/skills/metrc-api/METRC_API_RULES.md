...
## BudTags Payload Optimizations

BudTags implements backend payload optimizations to reduce frontend memory usage while maintaining full Metrc API compatibility.

### Optimized Types

| Type | Method | Fields Removed | Reduction | Performance Gain |
|------|--------|----------------|-----------|------------------|
| **Package** | `optimizePackagePayload()` | 22 / 90 fields | 27.5% | 2.64 MB saved (2,229 packages) |
| **Item** | `optimizeItemPayload()` | 43 / 67 fields | 64% | Lazy-loaded on demand |
| **Harvest** | `optimizeHarvestPayload()` | 15 / 31 fields | 48% | ~35-45 KB per page load |

### Not Optimized

- **Transfer**: Field count already minimal, optimization not beneficial
- **Plant**: Minimal performance gains, skipped

### Implementation

**Backend:**
```php
// In app/Http/Controllers/MetrcController.php
protected function optimizePackagePayload(array $package): array {
    // Removes 22 rarely-used fields
    // Source tracking, lab testing, sample flags, etc.
    $excludedFields = [
        'SourceHarvestCount', 'SourcePackageCount', 'SourceProcessingJobCount',
        'SourceHarvestNames', 'InitialLabTestingState', 'LabTestingPerformedDate',
        // ... 16 more fields
    ];

    return collect($package)->except($excludedFields)->all();
}
```

**Frontend:**
```typescript
// In resources/js/Types/types-metrc.tsx
export type Package = {
    Id: number;
    Label: string;
    // ... core fields always present

    IsProductionBatch?: boolean;  // Excluded from payload
    ProductionBatchNumber?: string;  // Excluded from payload
...
**Package (22 fields):**
- Source tracking (4): SourceHarvestCount, SourcePackageCount, SourceProcessingJobCount, SourceHarvestNames
- Lab testing (6): InitialLabTestingState, LabTestingPerformedDate, LabTestResultExpirationDateTime, LabTestingRecordedDate, LabTestStageId, LabTestStage
- Sample flags (3): IsTradeSamplePersistent, SourcePackageIsTradeSample, IsProcessValidationTestingSample
- Donation flags (2): IsDonationPersistent, SourcePackageIsDonation
- Dates (2): RemediationDate, DecontaminationDate
- Decontamination (2): ProductRequiresDecontamination, ContainsDecontaminatedProduct
- Other (3): IsOnRetailerDelivery, PackageForProductDestruction, Note

**Item (43 fields):**
- Product specifications, serving sizes, cannabinoid limits, extensive product metadata
- See full list: `app/Http/Controllers/MetrcController.php::optimizeItemPayload()`

**Harvest (15 fields):**
- Drying location details, growth phase metadata, auxiliary tracking fields
- See full list: `app/Http/Controllers/MetrcController.php::optimizeHarvestPayload()`

### Accessing Excluded Fields

**When needed, query Metrc API directly:**
```php
// Get complete package with all fields
$fullPackage = $api->get("/packages/v2/{$label}?licenseNumber={$license}");
// Contains all 90+ Package fields as defined by Metrc
```

### Documentation

- **Performance Analysis**: `.claude/docs/performance/payload-field-optimization-measurement-code.md`
